...
namespace ESKF_VIO_BACKEND {
    ...
    bool Backend::Initialize(const std::string &configPath) {
        ...
        if (this->LoadMatrix(configPath + "/imu_na_ng_nwa_nwg.txt", 1, 4, tempMat) == true) {
            ...
            std::cout << "     imu accel noise : " << this->propagator.Q(INDEX_NA, INDEX_NA) << "\n";
            std::cout << "     imu gyro noise : " << this->propagator.Q(INDEX_NG, INDEX_NG) << "\n";
            std::cout << "     imu accel random walk : " << this->propagator.Q(INDEX_NWA, INDEX_NWA) << "\n";
            std::cout << "     imu gyro random walk : " << this->propagator.Q(INDEX_NWG, INDEX_NA) << "\n";
        }
        ...
        std::cout << "     imu bias_a init : " << this->propagator.bias_a.transpose() << "\n";
        std::cout << "     imu bias_g init : " << this->propagator.bias_g.transpose() << "\n";
        ...
        std::cout << "     imu init nominal p_wb : " << this->propagator.initState.p_wb.transpose() << "\n";
        std::cout << "     imu init nominal v_wb : " << this->propagator.initState.v_wb.transpose() << "\n";
        std::cout << "     imu init nominal q_wb : [" << this->propagator.initState.q_wb.w() << ", " <<
